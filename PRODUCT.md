...
- **The journey page (how-we-work, 2026-09-23).** A page that tells a sequence in time is a run of stops: each stop is a clock (Today, Within 24 hours, Within 48 hours, Before day one, Day one, Month three, Years later) on the trunk's narrow side and its story across the trunk. Clocks are Grotesk at the CTA scale and brighten along the journey, from the kicker grey to white; stories take the face of their side (right of the trunk: Zilla), not of their topic. From 1200px the trunk sits at a quarter of the frame with the whole tree mirrored so canopy and roots open into the margin, and hero, stories and CTA share the wide side; below 1200px the page is an ordinary two-sided page, and under 700px the clock sits above its story. No branches on stops: they read as decoration. Rejected on the way: tool shapes (rows, facts) for this page ("hostile"), zigzag stops, an inverted scroll that climbs from the roots (it would need the home's engine).
