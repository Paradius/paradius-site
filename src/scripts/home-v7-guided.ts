/**
 * Guided language (desktop, any width): hijacked inverted scroll.
 * Geometry is measured ONCE per layout epoch (mount / reflow) and derived
 * arithmetically per frame, so the frame path performs zero layout reads.
 * Style writes are quantized and dirty-checked.
 */
...
function env(): LifecycleEnv {
  return { viewportH, current, hijack: true };
}
...
function maybeSnap(dt: number): void {
  ...
  if (!snapEnabled || !snapArmed || !ascending) return;
...
function boxes(): Box[] {
  return reveals.map((item) => ({ top: item.docTop, bottom: item.docBottom }));
}

export function createGuidedEngine(): HomeEngine<BoxAnchor | null> {
  return {
    mount() {
    ...
    captureAnchor() {
      return captureBoxAnchor(boxes(), current + viewportH / 2);
    },
    restoreAnchor(anchor) {
      if (!anchor) return;
      target = restoreBoxAnchor(boxes(), anchor, viewportH / 2, maxScroll);
      current = target;
      applyFrame();
    },
    setFrozen(next) {
      frozen = next;
      if (!next) lastTickAt = 0;
    },
  };
}
