...
// PROTOTYPE (?flip): the touch home does not scroll, it turns pages.
const FLIP_MS = 420;
...
// Inline transform on the tree layers only: a --flip-p write on the home root
...
function goTo(next: number): void {
  ...
  lightNear();
  document.documentElement.removeAttribute('data-flip-drag');
  setOffset(stops[index]);
  setTree(journeyProgress(index));
  homeRoot?.style.setProperty('--flip-p', String(journeyProgress(index)));
  if (nearTimer) window.clearTimeout(nearTimer);
  nearTimer = window.setTimeout(markNear, FLIP_MS + 60);
}
...
function onTouchMove(e: TouchEvent): void {
  ...
  document.documentElement.setAttribute('data-flip-drag', '');
...
export function createFlipEngine(): HomeEngine<number> {
  return {
    mount() {
      ...
      document.documentElement.setAttribute('data-pager', 'flip');
      document.documentElement.style.setProperty('--flip-ms', `${FLIP_MS}ms`);
      window.scrollTo(0, 0);
      measure();
      goTo(0);
      markNear();
